...
import { getOffers, getActiveOffers, createOffer } from '../_db.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const {
        advertiserId, title, description, discountType, discountValue,
        offerUrl, commissionRate, conversionRate, avgOrderValue, category,
      } = req.body;
      ...
      const offer = await createOffer({
        advertiserId, title, description, discountType,
